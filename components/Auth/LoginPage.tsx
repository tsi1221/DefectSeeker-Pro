...
const LoginPage: React.FC<LoginPageProps> = ({ users, onLogin }) => {
  ...
  const handleFormLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // In MVP, we just match email for simplicity, but acknowledge password exists
    const user = users.find(u => u.email === email);
    if (user) {
      onLogin(user);
      navigate('/');
    } else {
      setError('Invalid system credentials. Please check your corporate ID.');
    }
  };
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 font-medium tracking-wide">Enterprise MVP Quality Control</p>
        </div>
        ...
        <div className="bg-white/95 backdrop-blur-2xl rounded-[3rem] p-10 shadow-2xl border border-white/20">
          <form onSubmit={handleFormLogin} className="space-y-6">
            <div>
              ...
              <div className="relative group">
                ...
                <input 
                  type="email" 
                  required
                  placeholder="admin@defectseeker.pro"
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2.5 ml-1">System Secret</label>
              <div className="relative group">
            ...
            <button 
              type="submit"
              className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-xl shadow-indigo-500/20 hover:bg-indigo-700 hover:-translate-y-1 transition-all"
            >
              Initialize Session
            </button>
          </form>
          ...
          <div className="mt-8 pt-8 border-t border-slate-100">
            <h3 className="text-center text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Demo Role Selection</h3>
            <div className="grid grid-cols-2 gap-2">
              {users.map(user => (
                <button
                  key={user.id}
                  onClick={() => { onLogin(user); navigate('/'); }}
                  className="flex items-center gap-2.5 p-3 rounded-2xl border border-slate-100 hover:border-indigo-600 hover:bg-indigo-50/50 transition-all text-left group"
                >
                  <img src={user.avatar} alt={user.name} className="w-8 h-8 rounded-full bg-slate-100" />
                  <div className="min-w-0">
                    <p className="font-bold text-slate-900 text-[10px] truncate">{user.name}</p>
